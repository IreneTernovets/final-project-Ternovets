...
function displayForecast(respond) {
  ...
  let forecastHTML = `<h3 class="sidebar-header">Forecast</h3> <div class="weather-forecast-item">`;
  forecast.forEach(function (forecastDay, index) {
    if (index < 4) {
      forecastHTML += `<ul class="weather-forecast-list">
    <li class="forecast-day">${formatDate(forecastDay.dt)}</li>
    <li><img src="http://openweathermap.org/img/wn/${
      forecastDay.weather[0].icon
    }@2x.png" alt="" width = "50"></li>
    <li class="forecast-temperature"><span class="forecast-temp-max">${Math.round(
...
function displayCurrent(response) {
  ...
  temperature.innerHTML = `${formatTemp(Math.round(response.data.main.temp))}°`;
  ...
  date.innerHTML = transformedDate(response.data.dt);
